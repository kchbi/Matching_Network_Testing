...
void Motor_Set(Motor_ID_t motor, float command)
{
    ...
    if (command > 0.5f) {
         ...
         * Motor free-wheels. No braking. PID can re-drive
         * smoothly without fighting a brake. */
    ...
    /* Clamp to max */
    if (abs_cmd > PWM_MAX_DUTY) abs_cmd = PWM_MAX_DUTY;

    /* Enforce minimum drive to overcome stiction */
    if (abs_cmd < PWM_MIN_DUTY) abs_cmd = PWM_MIN_DUTY;
