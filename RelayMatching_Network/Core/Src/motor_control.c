                     ...
                     uint8_t reset)
{
    if (reset) {
        ...
        pid->prev_tick = HAL_GetTick();
        return 0.0f;
    }

    float dt = (HAL_GetTick() - pid->prev_tick) / 1000.0f;
                            ...
                            uint16_t tolerance)
{
    ...
    if (motor == MOTOR_1) Max_PCurrent_M1 = 0;
    else                  Max_PCurrent_M2 = 0;

    PID_Run(pid, 0, Pot1_2[motor], 1);

    while (1) {
        uint16_t curADC = Pot1_2[motor];
        int32_t error = (int32_t)targetADC - curADC;

        I2C_ReadCurrent(&hi2c1,
                        motor == MOTOR_1 ? Motor1 : Motor2,
                        &instCurrent);

        float absI = fabs(instCurrent);
        if (motor == MOTOR_1) {
        ...
        int32_t *max_error_out)
{
    ...
    *max_error_out = 0;
    PID_Run(pid, 0, startADC, 1);

    while (1) {
        uint16_t cur = Pot1_2[motor];
        int32_t error = targetADC - cur;
        ...
        if (expected_duration_ms > 0) {
            int32_t ideal =
                startADC +
                ((int32_t)(targetADC - startADC) * elapsed) /
                expected_duration_ms;

            int32_t smooth_err = cur - ideal;
            if (labs(smooth_err) > labs(*max_error_out))
                *max_error_out = smooth_err;
        }

        float cmd = PID_Run(pid, (float)error, (float)cur, 0);
        Motor_Set(motor, cmd);
...
{
    uint32_t duration = MotorControl_MinToMax(motor);
    ...
    float dist = motorCal[motor].adc_max - motorCal[motor].adc_min;
    return (dist > 0) ? ((float)err / dist) : 0.0f;
}

float MotorControl_GetSmoothness_MaxToMin(Motor_ID_t motor)
{
    uint32_t duration = MotorControl_MaxToMin(motor);
    HAL_Delay(SETTLE_DELAY_MS);
