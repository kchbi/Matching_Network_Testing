...
{
    uint32_t total_time_ms = (uint32_t)(avg_count * conv_time_ms_per_sample + 0.5f);
    uint32_t start_tick = HAL_GetTick();
    uint8_t rx_buffer[2];
    uint16_t mask;

    while ((HAL_GetTick() - start_tick) < total_time_ms)
    {
        ...
        mask = (rx_buffer[0] << 8) | rx_buffer[1];

        if (mask & CVRF_BIT)
            return HAL_OK;  // Conversion is ready

        HAL_Delay(1);  // Wait a bit before polling again
    }
