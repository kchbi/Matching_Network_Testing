...
#define ADC_MOTOR1_INDEX   0
#define ADC_MOTOR2_INDEX   1
 ...
 */
static MotorCalibration_t motorCal[2] = { { 370, 3250, 0 }, // MOTOR_1 defaults at 370 it Just hits the switch .
...
void MotorControl_ServiceI2C(Motor_ID_t motor) {
	...
	if (HAL_GetTick() - last_tick[motor] < 5)
		return;   // gate to 50 Hz
	...
	if (I2C_ReadCurrent(&hi2c1, T24V, &I24) == HAL_OK) {
		float a = fabsf(I24);
		if (motor == MOTOR_1) {
			if (a > Max_24VCurrent_M1)
				Max_24VCurrent_M1 = a;
		} else {
			if (a > Max_24VCurrent_M2)
				Max_24VCurrent_M2 = a;
		}
	}
	if (I2C_ReadCurrent(&hi2c1, T15VP, &I15P) == HAL_OK) {
		float a = fabsf(I15P);
		if (motor == MOTOR_1) {
			if (a > Max_PCurrent_M1)
				Max_PCurrent_M1 = a;
		} else {
			if (a > Max_PCurrent_M2)
				Max_PCurrent_M2 = a;
		}
	}
...
//    float a    = fabsf(P15N / 15.0f);
	float a = 0.0f;
	if (motor == MOTOR_1) {
		if (a > Max_NCurrent_M1)
			Max_NCurrent_M1 = a;
	} else {
		if (a > Max_NCurrent_M2)
			Max_NCurrent_M2 = a;
	}
}
...
void MotorControl_MoveToADC(Motor_ID_t motor, uint16_t targetPOS,
	...
	while (1) {
		...
		if (abs((int32_t) currADC - (int32_t) targetADC) <= MOVE_TOLERANCE_ADC) {
			stable_count++;
...
void MotorControl_MoveAndMeasureSmoothness(Motor_ID_t motor, uint16_t targetPOS,
		uint16_t targetADC, uint32_t expected_duration_ms,
	...
	*max_error_out = 0;
	...
	while (1) {
		...
		if (expected_duration_ms > 0 && elapsed <= expected_duration_ms) {
			...
			int32_t smooth_err = (int32_t) currADC - ideal;
...
float MotorControl_FindMax(Motor_ID_t motor) {
	...
	Motor_Set(motor, MAX_POS-200);
	HAL_Delay(4000);
	Motor_Set(motor,MAX_POS-100);
	HAL_Delay(500);
	...
	voltage = ((float) motorCal[motor].adc_max * 12.33f) / 4095.0f;
...
float MotorControl_FindMin(Motor_ID_t motor) {
	...
	Motor_Set(motor, MIN_POS+200);
	HAL_Delay(4000);
	Motor_Set(motor,MIN_POS+100);
	HAL_Delay(500);
	...
	voltage = ((float) motorCal[motor].adc_min * 12.33f) / 4095.0f;

	return voltage;
}

void MotorControl_UpdateHome(void) {
	motorCal[MOTOR_1].adc_home = (motorCal[MOTOR_1].adc_min
			+ motorCal[MOTOR_1].adc_max) / 2;

	motorCal[MOTOR_2].adc_home = (motorCal[MOTOR_2].adc_min
			+ motorCal[MOTOR_2].adc_max) / 2;

	MotorControl_MoveToADC(MOTOR_1,
	MID_POS, motorCal[MOTOR_1].adc_home);

	MotorControl_MoveToADC(MOTOR_2,
	MID_POS, motorCal[MOTOR_2].adc_home);
}
...
float MotorControl_GetSmoothness_MinToMax(Motor_ID_t motor) {
	...
	uint32_t duration = MotorControl_MinToMax(motor, &pP, &pN, &pV);
	...
	MotorControl_MoveAndMeasureSmoothness(motor,
	MAX_POS, motorCal[motor].adc_max, duration, &max_err);

	float dist = (float) (motorCal[motor].adc_max - motorCal[motor].adc_min);

	return (dist > 0.0f) ? ((float) labs(max_err) / dist) : 0.0f;
}
...
float MotorControl_GetSmoothness_MaxToMin(Motor_ID_t motor) {
	...
	uint32_t duration = MotorControl_MaxToMin(motor, &pP, &pN, &pV);
	...
	MotorControl_MoveAndMeasureSmoothness(motor,
	MIN_POS, motorCal[motor].adc_min, duration, &max_err);
